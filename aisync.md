...
- 2026-03-18: Reduced live PPP by 10% and rounded the authoritative values in `ltm3`: easy `0.000342`, medium `0.000468`, hard `0.000684`. Synced workbook, generator, and generated runtime config.
- 2026-03-18: Fixed weekly leaderboard difficulty filtering so filtered views now recalculate `games`, `wins`, `net`, and `tickets` from claimed games in the selected difficulty instead of only changing ticket totals. Also fixed `UserPageHeader` navigation so rules/leaderboard/back actions work consistently outside the home shell.
- 2026-03-18: Raised live PPP by 5% and rounded to easy `0.000359`, medium `0.000491`, hard `0.000718`. Synced `ltm3`, generator, and runtime config. Home difficulty cards now show BF net per point for each difficulty instead of only the currently selected summary.
