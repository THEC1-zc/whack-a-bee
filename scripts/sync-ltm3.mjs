...
const DIFFICULTY_META = {
  easy: { label: "Easy", emoji: "🟢", color: "#16a34a", fee: 0.01, baseWaves: 14, ppp: 0.0003045616875 },
  medium: { label: "Medium", emoji: "🟡", color: "#ca8a04", fee: 0.02, baseWaves: 13, ppp: 0.000464585625 },
  hard: { label: "Hard", emoji: "🔴", color: "#dc2626", fee: 0.03, baseWaves: 8, ppp: 0.000712364625 },
};
...
const MEDIUM_RULES = {
  waveDurationScale: 0.85,
  maxButterfliesAdd: { low: 0, nice: 0, big: 1, mega: 1 },
  tripleMaxGameAdd: { low: 1, nice: 1, big: 1, mega: 2 },
  tripleMaxWave: { low: 2, nice: 2, big: 2, mega: 3 },
  tripleChanceScale: 1.1,
  triplePoints: 4,
  tripleDurationScale: 0.9,
  quickMaxGameAdd: { low: 1, nice: 1, big: 1, mega: 2 },
  quickMaxWave: { low: 1, nice: 1, big: 1, mega: 2 },
  quickChanceScale: 1.15,
  quickPoints: 5,
  quickDurationScale: 0.9,
  prizeDurationScale: 0.9,
  normalDurationScale: 0.9,
  bombsBase: { low: 1, nice: 1, big: 1, mega: 2 },
  bombsSecondAdd: 0.1,
  bombPoints: -2,
  bombDurationScale: 0.9,
};

const HARD_RULES = {
  waveDurationScale: 0.72,
  maxButterfliesAdd: { low: 1, nice: 1, big: 2, mega: 2 },
  tripleMaxGameAdd: { low: 2, nice: 2, big: 2, mega: 3 },
  tripleMaxWave: { low: 2, nice: 2, big: 3, mega: 3 },
  tripleChanceScale: 1.2,
  triplePoints: 5,
  tripleDurationScale: 0.82,
  quickMaxGameAdd: { low: 2, nice: 2, big: 2, mega: 3 },
  quickMaxWave: { low: 1, nice: 1, big: 2, mega: 2 },
  quickChanceScale: 1.3,
  quickPoints: 6,
  quickDurationScale: 0.78,
  prizeDurationScale: 0.78,
  normalDurationScale: 0.78,
  bombsBase: { low: 1, nice: 1, big: 2, mega: 2 },
  bombsSecondAdd: 0.2,
  bombPoints: -3,
  bombDurationScale: 0.78,
};
...
function buildDerivedDifficulty(easyMatrix, difficulty, rules) {
  const meta = DIFFICULTY_META[difficulty];
  const next = {};
  for (const type of RUN_TYPES) {
    const easy = easyMatrix[type];
    const extraWaves = toNumber(easy.extra_waves);
    const totalWaves = meta.baseWaves + extraWaves;
    next[type] = {
      fee_usdc: String(meta.fee),
      base_waves: String(meta.baseWaves),
      extra_waves: String(extraWaves),
      total_waves: String(totalWaves),
      wave_duration_ms: String(Math.round(toNumber(easy.wave_duration_ms) * rules.waveDurationScale)),
      max_butterflies_per_wave: String(toNumber(easy.max_butterflies_per_wave) + rules.maxButterfliesAdd[type]),
      triple_max_per_game: String(toNumber(easy.triple_max_per_game) + rules.tripleMaxGameAdd[type]),
      triple_max_per_wave: String(rules.tripleMaxWave[type]),
      triple_chance_per_wave: String(round(toNumber(easy.triple_chance_per_wave) * rules.tripleChanceScale, 4)),
      triple_points: String(rules.triplePoints),
      triple_duration_ms: String(Math.round(toNumber(easy.triple_duration_ms) * rules.tripleDurationScale)),
      quick_max_per_game: String(toNumber(easy.quick_max_per_game) + rules.quickMaxGameAdd[type]),
      quick_max_per_wave: String(rules.quickMaxWave[type]),
      quick_chance_per_wave: String(round(toNumber(easy.quick_chance_per_wave) * rules.quickChanceScale, 4)),
      quick_points: String(rules.quickPoints),
      quick_duration_ms: String(Math.round(toNumber(easy.quick_duration_ms) * rules.quickDurationScale)),
      prize_max_per_game: "1",
      prize_chance: "0.01",
      prize_points: "1",
      prize_bonus_usdc_gross: "0",
      prize_duration_ms: String(Math.round(toNumber(easy.prize_duration_ms) * rules.prizeDurationScale)),
      normal_points: "1",
      normal_duration_ms: String(Math.round(toNumber(easy.normal_duration_ms) * rules.normalDurationScale)),
      bombs_base_per_wave: String(rules.bombsBase[type]),
      bombs_second_chance: String(round(Math.min(0.95, toNumber(easy.bombs_second_chance) + rules.bombsSecondAdd), 4)),
      bomb_points: String(rules.bombPoints),
      bomb_duration_ms: String(Math.round(toNumber(easy.bomb_duration_ms) * rules.bombDurationScale)),
      ppp_input_usdc_per_point: String(meta.ppp),
      bf_per_usdc_live_snapshot: easy.bf_per_usdc_live_snapshot || "",
      run_roll_pct: String(RUN_TYPE_ROLL_PCT[type]),
    };
  }
  return next;
}
